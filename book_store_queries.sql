...
SELECT *
FROM (	SELECT cust_id, MAX(book_amount) max_book_amount
		FROM (	SELECT cust_id, COUNT(cust_id) book_amount
				FROM (	SELECT *
						FROM purchases
...
SELECT *
FROM (	SELECT supp_id, max(amount)
		FROM (	SELECT supp_id, SUM(amount) AS amount
				FROM orders
...
FROM (	SELECT origin_price - cust_pay AS discount
		FROM (	SELECT *
				FROM purchases
...
SELECT *
FROM (	SELECT *
		FROM (	SELECT book_id, COUNT(*) AS amount
				FROM purchases
				WHERE purch_date BETWEEN 'fromDate' AND 'tilDate'
				GROUP BY book_id)
